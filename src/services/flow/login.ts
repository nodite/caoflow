...
export default class LoginService extends BaseService {
  ...
  public async authPortal(): Promise<void> {
    ...
    cbApp.use('/auth/callback', async (req, res) => {
      ...
      const tenantUrl = new URL(`${FLOW_BASE_URL}/login/tenants`)
    ...
    const loginBrowser = await puppeteer.core.launch({
      args: [
        `--app=${FLOW_BASE_URL}/login?redirect_uri=${generatePrincipalPolicyUrlDto.redirectUri}`,
        '--window-size=600,800',
